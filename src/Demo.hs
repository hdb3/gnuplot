...
plot2d :: String -> ( Plot2D.T Int Double ) -> Frame.T (Graph2D.T Int Double)
plot2d t p =
   let lineSpec =
          Graph2D.lineSpec $
          LineSpec.title "runtimes" $
       ...
       frameOpts =
          Opts.xLabel "input size" $
          Opts.yLabel "runtime (ms)" $
          Opts.title ("Graph from " ++ t) $
          Opts.deflt
   in  Frame.cons frameOpts $ fmap lineSpec p

plot :: [(Int,Double)] -> ( Plot2D.T Int Double )
plot d = Plot2D.list Graph2D.lines d

plot1 :: Plot2D.T Int Double
plot1 = plot rawData
    where rawData =
            [
              ( 1 , 4.7431578947368424e-4 )
...
main :: IO ()
main = Plot.plot X11.cons ( plot2d "my first graph" plot1) >> return ()
--main = Plot.plot X11.cons ( file2d "data" "runtime.data" ) >> return ()

--main = sequence_ $
   --Plot.plot X11.cons ( file2d "data" "runtime.data" ) :
   --[]
